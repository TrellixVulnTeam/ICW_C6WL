...
#time of caculation in the algorithmus
import time
#data set
...
def generate_dataset(n_points_per_cluster_total=100000, size_colum=100):
    """
        This function generate data set for allgorithmus. That based on the function make_blobs of sklearn
    Parameters
    ----------
    n_points_per_cluster_total : int, the number of all point of cluster, eg: 1000 is 1000 element
    size_colum: the highdimenstion's number .eg: 100 - means 100 dimension
    """
    centers = np.random.randint(-100, 100, size=(size_colum, size_colum))

    X, labels_true = make_blobs(n_samples=n_points_per_cluster_total, centers=centers, n_features=size_colum,
...
print('Elapsed time to cluster in Denclue algorithmus :  %.4f s ' % db_time_process)
labels = den1.fit_predict(X)
