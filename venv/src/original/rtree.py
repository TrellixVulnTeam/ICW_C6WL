#source : https://github.com/sulaiman-muhammad/R-Tree/blob/master/rtree.py
class Point:
...
class Rectangle:
    ...
    def is_present(self, item):
        ...
        if (type(item) == Point):
            ...
            if (
                    self.ux <= item.ux <= self.lx and self.ly <= item.uy <= self.uy and self.ux <= item.lx <= self.lx and self.ly <= item.ly <= self.uy):
                return True
        return False
    ...
    def pick_seeds(self, bb):
        ...
        for i in range(5):
            for j in range(i + 1, 5):
                ...
                d = area - ((bb[i].lx - bb[i].ux) * (bb[i].uy - bb[i].ly)) - (
                            (bb[j].lx - bb[j].ux) * (bb[j].uy - bb[j].ly))
                if d_max < d:
                    d_max = d
                    p1 = i
                    p2 = j
