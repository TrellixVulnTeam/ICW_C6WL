...
X, labels_true = make_var_density_blobs(n_samples=1000, centers=centers, cluster_std=densities,
                                        random_state=0)

X = StandardScaler().fit_transform(X)
dist = np.linalg.norm(X[1]-X[2])
dist2 = np.linalg.norm(X[5]-X[6])
dist3 = np.linalg.norm(X[6]-X[7])
dist4 = np.linalg.norm(X[7]-X[8])
dist5 = np.linalg.norm(X[8]-X[9])
print(dist)
print(dist2)
print(dist3)
print(dist4)
